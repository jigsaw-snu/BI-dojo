# https://www.tandfonline.com/doi/full/10.1080/26895293.2022.2078896
# https://github.com/BCMSLab/obese_mice/tree/master/scripts
...
tissues <- as.vector(unique(meta_data$Tissue))

dds <- DESeq2::DESeqDataSetFromMatrix(countData = count_data[, seq(12)],
                                      colData = meta_data[seq(12), ],
                                      design = ~Genotype * Diet)
...
pca_df <- merge(pca_df, meta_data[seq(12), ], by = "row.names") %>%
    tibble::column_to_rownames(var = "Row.names")

ggplot2::ggplot(data = pca_df, aes(x = PC1, y = PC2)) +
    ggplot2::geom_point(size = 5, aes(color = Group)) +
